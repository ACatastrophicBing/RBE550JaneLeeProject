...
class DStar:
    ...
    def process_state(self):
        ...
        node_k = self.get_k_min()
        self.delete(node)
        if node_k<node.h:
            for neighnor in neighbours:
                if neighnor.h<=node_k and (node.h>(neighnor.h+self.cost(neighnor,node))):
                    node.parent=neighnor
                    node.h = neighnor.h+self.cost(neighnor,node)
        ...
        if node_k == node.h:
            ...
            for n in neighbours:
                if n.tag == "NEW" or n.parent == node and n.h != (node.h + self.cost(n, node)):
                    n.parent = node
                    self.insert(n,node.h+self.cost(n,node))
                else:
                    if n.parent != node and (n.h>node.h+self.cost(n,node)):
                        self.insert(node,node.h)
                    else:
                        if n.parent!=node and (node.h>n.h+self.cost(n,node)) and n.tag == "CLOSED" and (n.h>node_k):
                            self.insert(n,n.h)
    ...
    def prepare_repair(self, node):
        ...
        for neighbor in neighbors:
            ...
            if neighbor.is_dy_obs == True and neighbor.is_obs == False:            
                ...
                #since the new dynamic neighbor is a obstacle the cost will be infinity
                self.insert(neighbor, math.inf)
                n = self.get_neighbors(neighbor)
                for new_n in n:
                    self.modify_cost(node, new_n)
