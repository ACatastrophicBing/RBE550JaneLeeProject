...
class RRT:
    ...
    def get_new_point_in_ellipsoid(self, goal_bias, c_best):
        ...
        # Generate a random point in an ellipsoid
            ...
            rotation_angle = np.arctan2((self.goal.col - self.start.col),(self.goal.row - self.start.row))
            C = np.array([[np.cos(rotation_angle), -1*np.sin(rotation_angle)],[np.sin(rotation_angle), np.cos(rotation_angle)]]) # Rotation about z axis (in-plane)
            # Compute diagonal matrix - L
            L = np.array([[c_best/2, 0], [0, np.sqrt(c_best**2 - c_min**2)/2]])
            # Cast a sample from a unit ball - x_ball
            x_ball = np.random.normal(scale=1.0,size=(2,1))
            # Map ball sample to the ellipsoid - x_rand
            CL = np.matmul(C,L)
            x_rand = np.matmul(CL,x_ball) + x_center
            point = [x_rand[0,0], x_rand[0,1]]
        #### TODO END ####
